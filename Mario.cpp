...
void CMario::Render()
{
	...
	else if (level == MARIO_LEVEL_FIRE)
	{
		...
		if (fire == true)
		{
			int ani_fire;
			ani_fire = ANI_FIRE;
			animations[ani_fire]->Render(x_fire, y_fire, alpha);
		}
	}
...
void CMario::SetState(int state)
{
	...
	case MARIO_STATE_FIRE_RIGHT:
		if (fire == true)
		{
			if (vx_fire == 0)
			{
				x_fire = x;
				y_fire = y;
				vx_fire = SPEED_FIRE_X;
				vy_fire = SPEED_FIRE_Y;
			}
			else
			{
				vx_fire = SPEED_FIRE_X;
	...
	case MARIO_STATE_FIRE:
		if (nx == 1)
			SetState(MARIO_STATE_FIRE_RIGHT);
		else if (nx == -1)
			SetState(MARIO_STATE_FIRE_LEFT);
	}
}
