...
vector<LPGAMEOBJECT1> objectsBrick;
...
// LOAD BLOCK GREEN dau gach mau xanh
	...
	for (int i = 0; i < 50; i++)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			brick->AddAnimation(j);
			//brick1->AddAnimation(1);
			//brick2->AddAnimation(2);
			brick->SetPosition(0 * j + i * 32.0f, 416.0f);
		}
		//brick1->SetPosition(16 + i * 32.0f, 416.0f);
		//brick2->SetPosition(32 + i * 32.0f, 416.0f);
...
void Update(DWORD dt)
{
	...
	vector<LPGAMEOBJECT1> coObjectsBrick;
	for (int i = 1; i < objectsBrick.size(); i++)
	{
		coObjectsBrick.push_back(objectsBrick[i]);// thêm từ 1 vào 
	}
	...
	{
		objectsBrick[i]->Update(dt, &coObjectsBrick);
	}
	...
	cy -= SCREEN_HEIGHT / 2;
	if (cy < 0) cy = 0;
	if (cy > MAX_MAP_Y*16 - 202) cy = MAX_MAP_Y * 16 - 202;

	if (cx < 0) cx = 0;
	if (cx > MAX_MAP_X*16 - 305) cx = cx > MAX_MAP_X * 16 - 305;
